...
const VideocardBlock = () => {
    ...
    useEffect(() => {
        try {
            instanceAxios.get(`/brands?category_id=2`).then(res => {
                if (filterConfig['videocards'][0].options && filterConfig['videocards'][0].options.length === 0) {
                    res.data.data.forEach(el => {
                        filterConfig['videocards'][0].options?.push({ value: el.id, label: el.title })
                    });
                }
            })
            
            instanceAxios.get(`/memory-generations`).then(res => {
                if (filterConfig['videocards'][1].options && filterConfig['videocards'][1].options.length === 0) {
                    res.data.data.forEach(el => {
                        filterConfig['videocards'][1].options?.push({ value: el.id, label: el.title })
                    });
                }
            })
    ...
    const onApplyFilters = async () => {
        ...
        const response = await instanceAxios.get(`/graphic-cards?${query}`);
    ...
    const handleComponentClick = async () => {
        try {
            console.log(socket)
            await instanceAxios.get(`/graphic-cards${hasProcessor ? `?socket_id=${socket}` : ""}`).then(res => {
                ...
                Object.keys(currentComponent).length !== 0 ? (
                    <>
                        <div className="text-center">
                            ...
                            <p className="">{currentComponent.gpu_model}</p>
                            {
                                        ...
                                        <>
                                            <div className="md:grid grid-cols-4 items-center my-[20px]">
                                                <div className="text-center">
                                                    ...
                                                    <p className="text-[20px] text-[#fffffff]">{el.gpu_model}</p>
                                                </div>
